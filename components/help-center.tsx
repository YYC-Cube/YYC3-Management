...
export function HelpCenter() {
  ...
  const getDifficultyBadge = (difficulty: HelpArticle["difficulty"]) => {
    switch (difficulty) {
      ...
      default:
        return <Badge className="bg-muted text-card-foreground">未知</Badge>
    }
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <Card className="bg-card/90 backdrop-blur-sm border border-primary/10 rounded-xl shadow-sm hover:shadow-xl hover:border-primary/20 transition-all duration-300 hover:scale-105 border-r-[5px] border-r-primary shadow-md">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-lg font-bold text-foreground">帮助文章</p>
        ...
        <Card className="bg-card/90 backdrop-blur-sm border border-primary/10 rounded-xl shadow-sm hover:shadow-xl hover:border-primary/20 transition-all duration-300 hover:scale-105 border-r-[5px] border-r-success shadow-md">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-lg font-bold text-foreground">视频教程</p>
        ...
        <Card className="bg-card/90 backdrop-blur-sm border border-primary/10 rounded-xl shadow-sm hover:shadow-xl hover:border-primary/20 transition-all duration-300 hover:scale-105 border-r-[5px] border-r-purple-500 shadow-md">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-lg font-bold text-foreground">常见问题</p>
          ...
          {filteredArticles.length > 0 ? (
              ...
              {filteredArticles.map((article) => (
                ...
                >
                  <CardContent className="p-6">
                    ...
                    <p className="text-muted-foreground mb-4 line-clamp-2">{article.content}</p>
            ...
            <Card className="bg-card/90 backdrop-blur-sm border border-primary/10 rounded-xl shadow-sm">
              <CardContent className="p-12 text-center">
                ...
                <h3 className="text-lg font-medium text-foreground mb-2">没有找到匹配的文章</h3>
                <p className="text-muted-foreground mb-4">请尝试使用不同的搜索词或选择其他分类</p>
          ...
          {filteredVideos.length > 0 ? (
            ...
            <Card className="bg-card/90 backdrop-blur-sm border border-primary/10 rounded-xl shadow-sm">
              <CardContent className="p-12 text-center">
                ...
                <h3 className="text-lg font-medium text-foreground mb-2">没有找到匹配的视频</h3>
                <p className="text-muted-foreground mb-4">请尝试使用不同的搜索词或选择其他分类</p>
                <Button variant="outline" onClick={() => setSearchTerm("")}>
                  清除搜索
                </Button>
          ...
          {filteredFAQs.length > 0 ? (
            ...
            <Card className="bg-card/90 backdrop-blur-sm border border-primary/10 rounded-xl shadow-sm">
              <CardContent className="p-12 text-center">
                ...
                <h3 className="text-lg font-medium text-foreground mb-2">没有找到匹配的常见问题</h3>
                <p className="text-muted-foreground mb-4">请尝试使用不同的搜索词或选择其他分类</p>
                <Button variant="outline" onClick={() => setSearchTerm("")}>
                  清除搜索
                </Button>
